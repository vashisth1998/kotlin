...
class SourceMapLocationRemapper(val sourceMaps: Map<String, SourceMap>) {
    ...
    private val visitor = object : RecursiveJsVisitor() {
        ...
        private fun remapNode(node: SourceInfoAwareJsNode) {
            val source = node.source as? JsLocation ?: return

            val sourceMap = sourceMaps[source.file] ?: return
            val group = sourceMap.groups.getOrElse(source.startLine) { return }
            if (group.segments.isEmpty()) return
